...
def load_faq_engine():
    ...
    # Path model menggunakan subfolder karena hasil ekstrak zip menghasilkan folder bersarang
    engine = FAQEngine(
        model_name_or_path='faq_sbert_finetuned/faq_sbert_finetuned',
        threshold=0.45
    )

    try:
        # Load dataset langsung dari file pickle hasil notebook Colab
        engine.load_data(pkl_path='data/output/faq_data.pkl')
    except FileNotFoundError:
        st.error("🚨 Error: File `faq_data.pkl` tidak ditemukan!")
        st.info("Pastikan file `faq_data.pkl` berada di folder `data/output/`.")
        st.stop()
...
if prompt := st.chat_input("Ketik pertanyaan Anda di sini..."):
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner("Sedang mencari jawaban terbaik..."):
            ...
            # Panggil metode pencarian lengkap dari FAQEngine (termasuk Top-3 matches)
            result = engine.retrieve_answer_with_matches(prompt, top_k=3)
        ...
        # Fitur Transparansi AI: Tampilkan Top-3 pertanyaan terdekat di dalam expander
        if result.get("matches"):
            # Jika fallback (OOD), ubah label expander menjadi saran agar lebih komunikatif
            expander_title = (
                "🔍 Mungkin maksud Anda pertanyaan ini?"
                if result.get("is_fallback")
                else "📋 Pertanyaan terkait"
            )
            with st.expander(expander_title):
                for idx, match in enumerate(result["matches"], start=1):
                    st.markdown(
                        f"{idx}. {match['question']} "
                        f"(**{match['score']:.2f}**)"
                    )
